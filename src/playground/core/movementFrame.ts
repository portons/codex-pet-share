...
export function applyMovementInput({
  ...
  if (moving) {
    ...
    sprite.position.x += worldDx * runSpeed * dt;
    sprite.position.z += worldDz * runSpeed * dt;
    const limit = FLOOR_HALF - SPRITE_WIDTH / 2;
    sprite.position.x = Math.max(-limit, Math.min(limit, sprite.position.x));
    sprite.position.z = Math.max(-limit, Math.min(limit, sprite.position.z));
  }
